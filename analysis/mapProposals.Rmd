...
joined.pres.norm %>% gather(type,value,`EFSA infections`,`Magarey\nasco_days\naverage`) %>%
    ...
    mutate(type = ifelse(type == "EFSA infections","rel. EFSA inf. score",type),
           type = ifelse(type == "Magarey\nasco_days\naverage","rel. Magarey inf. score",type)) %>%
    ggplot() +
    geom_bar(aes(x=Location,fill=type,y=value),stat="identity",position="dodge") +
